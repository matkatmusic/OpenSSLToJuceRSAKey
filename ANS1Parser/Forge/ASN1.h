                        ...
                        const int numNeeded)
{
    ...
    {
//        var error = new Error('Too few bytes to parse DER.');
        DBG( "Too few bytes to parse DER!" );
//        error.available = bytes.length();
        DBG( "available: " << bytes.getNumBytesRemaining() ) ;
//        error.remaining = remaining;
//        error.requested = n;
        DBG( "requested: " << numNeeded );
//        throw error;
        jassertfalse;
    }
}
...
template<typename StreamType>
int getInt(StreamType& bytes, int numBits)
{
    _checkBitsParam(numBits);
//    var rval = 0;
    int rval = 0;
    do
    {
...
//    var b2 = bytes.getByte();
    ...
    else
    {
        // the number of bytes the length is specified in bits 7 through 1
        // and each length byte is in big-endian base-256
        auto longFormBytes = b2 & 0x7F;
        _checkBufferLength(bytes, remaining, longFormBytes);
        length = getInt(bytes, longFormBytes << 3);
    ...
    {
//        throw new Error('Negative length: ' + length);
        DBG( "Error getting length: Negative length: " << length );
        jassertfalse;
        return -1;
    }
    return length;
...
struct ASNObject : juce::ReferenceCountedObject
{
    using Ptr = juce::ReferenceCountedObjectPtr<ASNObject>;
    Class tagClass;
    Type type;
    bool constructed = false;
    bool composed = false;
    /*
     TODO: decide on the correct data type for 'value'
     
     "The value's type depends on the constructed flag. If
     constructed, it will contain a list of other asn1 objects. If not,
     it will contain the ASN.1 value as an array of bytes formatted
     according to the ASN.1 data type."
     
     if it's constructed:
        I think I can use a vector of ASNObject::Ptr to represent the list
     otherwise
        use a MemoryBlock.
     
     With this decision in hand, update the 'create' function, and the usage in _fromDer, and fromDer
     */
    std::vector<Ptr> objectList;
...
typename ASNType::Ptr create(Class tagClass,
               Type type,
               bool constructed,
               std::vector<typename ASNType::Ptr> objectList,
               juce::MemoryBlock byteArray,
               ParseOptions::Ptr options)
{
    //remove nullptrs from objectList
    if(objectList.size() > 0)
    {
        std::vector<typename ASNType::Ptr> tmp;
    ...
    typename ASNType::Ptr obj = new ASNType();
    obj->tagClass = tagClass;
    obj->type = type;
    ...
    if(options && options->bitStringContents.getSize() > 0 )
    {
        obj->bitStringContents = options->bitStringContents;
    }
...
//            throw error;
    ...
    // value storage
//    var value;
    std::vector<ASNObject::Ptr> objectList; //value
    juce::MemoryBlock byteArray;
...
//        if(length === undefined)
        if( length == -1 )
        {
            ...
            for(;;)
            {
                ...
                start = bytes.length();
                value.push_back(_fromDer(bytes, remaining, depth + 1, options));
                remaining -= start - bytes.length();
            }
#endif
        }
...
//                start = bytes.length();
                auto start = bytes.getNumBytesRemaining();
                objectList.push_back(_fromDer<ASNType>(bytes, length, depth + 1, options));
//                remaining -= start - bytes.length();
                remaining -= start - bytes.getNumBytesRemaining();
...
//       type === asn1.Type.BITSTRING)
    std::unique_ptr<juce::MemoryBlock> bitStringContents;
    if( objectList.size() == 0 &&
...
//        bitStringContents = bytes.bytes(length);
        /*
         util.ByteStringBuffer.prototype.bytes uses 'this.data.slice(this.read)'
         'slice' could be a member function of:
         ...
         All of these 'slice' functions leave the original buffer/string intact without modifying the read index.
         */
...
//                };
                ...
                subOptions.strict = true;
                subOptions.decodeBitStrings = true;
                
//                var composed = _fromDer(bytes, remaining, depth + 1, subOptions);
                auto composed = _fromDer<ASNType>(bytes, remaining, depth + 1, subOptions);
...
//#if false
            /*
             'value' is treated as a UTF16 string or array of 16-bit ints here here.
             */
//            value = '';
...
//                value += String.fromCharCode(bytes.getInt16());
                auto numWritten = mos.writeFromInputStream(bytes, 2);
                jassert( numWritten == 2 );
                remaining -= 2;
            }
//#endif
            byteArray = data;
...
//    };
    ...
    ParseOptions::Ptr asn1Options = new ParseOptions();
    if( bitStringContents != nullptr )
    {
        asn1Options->bitStringContents = std::move(*bitStringContents);
    }
...
{
    /*
     this JS is unnecessary because of C++'s type system.
     */
//    // wrap in buffer if needed
//    if(typeof bytes === 'string')
//    {
